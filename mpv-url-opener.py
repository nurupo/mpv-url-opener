...
def verify_password(username, password):
    if username in config['HTTP_AUTH_DB'] and check_password_hash(config['HTTP_AUTH_DB'][username], password):
        ...
        print('Failed auth from {} for {}'.format(request.remote_addr, username))
...
def mpv_open_url():
    ...
    if not re.fullmatch(r'https://(www\.youtube\.com/watch\?v=|youtu.be/)[0-9a-zA-Z-_]{,16}', url):
        abort(400)
    print('{} ({}) has requested {}'.format(auth.current_user(), request.remote_addr, url))
...
def parse_args():
    ...
    parser.add_argument('ip', help='IPv4 address to listen on')
    parser.add_argument('port', type=int, help='Port to listen on')
    parser.add_argument('--ssl-cert', default='cert.pem', help='SSL/TLS public certificate file path')
    parser.add_argument('--ssl-key', default='key.pem', help='SSL/TLS private key file path')
    parser.add_argument('--config', default='config.json', help='Config file path')
    args = parser.parse_args()
    return args.ip, args.port, args.ssl_cert, args.ssl_key, args.config
...
if __name__ == '__main__':
    ip, port, cert, key, config_path = parse_args()
    ...
    print(f'Listening for requests on https://{ip}:{port}/mpv-open-url')
    waitress.serve(TransLogger(app), sockets=[s], threads=1, url_scheme='https')
    s.close()
